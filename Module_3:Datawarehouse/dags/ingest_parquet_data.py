...
def to_csv(file_name_parquet, file_name_csv):
...
LINK_PREFIX = "https://d37ci6vzurychx.cloudfront.net/trip-data/yellow_tripdata_"
OUT_FILE = "yellow_taxi_"
FORMAT1 = ".parquet"
FORMAT2 = ".csv"
DATE = "{{ execution_date.strftime(\'%Y-%m\') }}"
with csv_data_ingest:
    download_task = BashOperator(
        ...
        bash_command=f'curl -sSL {LINK_PREFIX + DATE + FORMAT1} > {OUT_FILE + DATE}' +  FORMAT1,
    ...
    conversion_task = PythonOperator(
        ...
        op_args=[
            "{{dag_run.dag.folder }}" + "/" + OUT_FILE + DATE + FORMAT1,
            "{{dag_run.dag.folder }}" + "/" + OUT_FILE + DATE + FORMAT2
        ]
    )
    ...
    delete_task = BashOperator(
        task_id='delete_parquet_and_csv',
        bash_command= f"rm {OUT_FILE + DATE + FORMAT1} {OUT_FILE + DATE + FORMAT2}",
        cwd = "{{ dag_run.dag.folder }}"
    )


    download_task >> conversion_task  >> create_table >>  migrate_task >> append_to_table >> [delete_table, delete_task]
